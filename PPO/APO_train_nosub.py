...
from APO_env_nosub import ChineseCheckersEnv
...
class TrainingLogger(BaseCallback):
    ...
    def plot(self):
        ...
        plt.tight_layout()
        plt.savefig("training_curves_nosub.png")
        plt.show()
        print("Saved to training_curves_nosub.png")
...
base_env = ChineseCheckersEnv()
base_env = Monitor(base_env)
env = ActionMasker(base_env, mask_fn)

logger = TrainingLogger()
run_name = os.environ.get("APO_RUN_NAME", "ppo_nosub")
...
checkpoint_callback = CheckpointCallback(
    ...
    name_prefix="checkers_ppo",
    verbose=1
)

print("Starting fresh training (no submoves)...")
model = MaskablePPO(
    "MlpPolicy",
    env,
    verbose=1,
...
model.save("checkers_ppo_final_nosub")
logger.plot()
