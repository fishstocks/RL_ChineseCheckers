...
def run_greedy_self_play(
    ...
    for game_idx in range(num_games):
        env = ChineseCheckersEnvMARL(
            num_players=2,
            render_mode="ansi" if (render_last or render_live) and game_idx == num_games - 1 else None,
            max_steps=max_steps,
        )
        _, info = env.reset()
        ...
        while not done:
            if render_live and game_idx == num_games - 1:
                _clear_screen()
                print(
                    f"2P Greedy self-play | game {game_idx + 1}/{num_games} | "
        ...
        if render_last and game_idx == num_games - 1:
            print(f"\nLast 2-player greedy game finished after {env.step_count} agent-turns")
            env.render()
            if "winner" in info:
    ...
    print(f"\n2-player greedy self-play summary over {num_games} games")
    print(f"Average agent-turns: {avg_steps:.2f}")
    print(f"Min/Max agent-turns: {min(step_counts)} / {max(step_counts)}")
    print(f"Truncations: {truncations}")
    print(f"Player 0 wins: {winner_counts[0]}")
    print(f"Player 1 wins: {winner_counts[1]}")


if __name__ == "__main__":
    num_games = int(os.environ.get("APO_BENCH_GAMES", "20"))
    max_steps = int(os.environ.get("APO_MAX_STEPS", "300"))
