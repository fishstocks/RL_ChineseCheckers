...
def build_env(num_players: int, max_steps: int):
    base_env = ChineseCheckersEnvMARL(num_players=num_players, max_steps=max_steps)
    base_env = Monitor(base_env)
    return ActionMasker(base_env, mask_fn)


def parse_args():
    parser = argparse.ArgumentParser(description="Long-running resumable PPO trainer for teacher-board-based shared-policy MARL.")
    parser.add_argument("--run-name", default=os.environ.get("APO_RUN_NAME", "ppo_marl_4p_1h"))
    parser.add_argument("--hours", type=float, default=float(os.environ.get("APO_TRAIN_HOURS", "1")))
    parser.add_argument("--num-players", type=int, default=int(os.environ.get("APO_NUM_PLAYERS", "4")))
    parser.add_argument("--save-freq", type=int, default=int(os.environ.get("APO_SAVE_FREQ", "50000")))
    parser.add_argument("--max-steps", type=int, default=int(os.environ.get("APO_MAX_STEPS", "400")))
    parser.add_argument("--total-timesteps", type=int, default=int(os.environ.get("APO_TOTAL_TIMESTEPS", "10000000")))
...
def main():
    args = parse_args()
    run_root = Path("longrun_training") / args.run_name
    ...
    env = build_env(args.num_players, args.max_steps)
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix=f"checkers_ppo_marl_{args.num_players}p",
        verbose=1,
    )
    ...
    if resume_path is not None:
        ...
        print("Starting fresh 4-player shared-policy training run.")
        model = MaskablePPO(
            "MlpPolicy",
            env,
            verbose=1,
