...
try:
    import pandas as pd
except ImportError:  # pragma: no cover - optional for round files only
    pd = None


# Game logic imports
ROOT_DIR = Path(__file__).resolve().parent
TEACHER_DIR = ROOT_DIR / "single system"
...
round_number = 1  # Update this for each new round of the tournament
...
class Game:
    def __init__(self):
        ...
        self.primary_colours = random.sample(PRIMARY_COLOURS, len(PRIMARY_COLOURS))
    ...
    def compute_scores(self):
        ...
        for pl in self.players:
            ...
            pins_in_goal = sum(
                1 for p in pins if self.board.cells[p.axialindex].postype == opposite
            )
            ...
            win_bonus = 1000.0 if pl.status == "WIN" else 0.0
            if pl.status == "WIN":
                final_score += win_bonus
        ...
        if SESSION.round_df is not None:
            ...
            if idx:
                idx = idx[0]
                SESSION.round_df.at[idx, "final_scores"] = ";".join(
                    f"{pl.name}:{self.scores[pl.player_id]['final_score']:.1f}" for pl in self.players
                )
                SESSION.round_df.at[idx, "status"] = self.status
                SESSION.round_df.at[idx, "winner"] = next(
                    (pl.name for pl in self.players if pl.status == "WIN"),
                    "None",
                )
                SESSION.round_df.at[idx, "time_scores"] = ";".join(
                    f"{pl.name}:{self.scores[pl.player_id]['time_score']:.1f}" for pl in self.players
                )
                SESSION.round_df.at[idx, "distance_scores"] = ";".join(
                    f"{pl.name}:{self.scores[pl.player_id]['distance_score']:.1f}" for pl in self.players
                )
                SESSION.round_df.at[idx, "pin_scores"] = ";".join(
                    f"{pl.name}:{self.scores[pl.player_id]['pin_goal_score']:.1f}" for pl in self.players
                )
                try:
...
class Session:
    def __init__(self):
        ...
        if pd is not None and os.path.exists(self.round_path):
    ...
    def find_round_game_for_player(self, player_name: str) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            candidate_rows["joined_count"] = candidate_rows.apply(
                lambda row: sum(
                    pd.notna(row[["player1", "player2", "player3", "player4", "player5", "player6"]])
                ),
                axis=1,
            )
            ...
            try:
                joined_players = {p.name for gid in self.session_games for p in self.games[gid].players}
                with open(f"joined_{self.round_number}.txt", "w", encoding="utf-8") as f:
                    for gid in self.session_games:
                        game = self.games[gid]
                        for p in game.players:
                            f.write(f"{p.name} joined game {gid} as {p.colour}\n")
                    not_joined = [
                        p for p in all_players_in_row if p != player_name and p not in joined_players
                    ]
                    f.write(
                        f"Players not joined yet for game {game_id}: {', '.join(not_joined) if not_joined else 'None'}\n"
                    )
            except Exception as e:
    ...
    def get_legal_moves(self, game_id: str, player_id: str):
        with self.lock:
            ...
            legal = {i: pin.getPossibleMoves() for i, pin in enumerate(pins)}
...
def handle_request(req: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if op == "get_legal_moves":
        return SESSION.get_legal_moves(req.get("game_id"), req.get("player_id"))

    if op == "join":
        if SESSION.round_df is None:
    ...
    while True:
        ...
        if cmd == "create":
            if SESSION.round_df is None:
                ...
                print(SESSION.create_round_games())
        elif cmd == "status":
